...
function Sidebar({ currentView, onNavigate, isCollapsed, currentUser }) {
  ...
  const isManufacturing = role === 'MANUFACTURING_USER';
  const isInventory = role === 'INVENTORY_MANAGER';
  const isOwner = role === 'BUSINESS_OWNER';

  // Section visibility based on role
  const canSeeSales = isAdmin || isSales || isOwner;
  const canSeePurchase = isAdmin || isPurchase || isOwner;
  ...
  const NavItem = ({ id, icon, label, style = {} }) => {
    ...
    return (
      <div 
        ...
        style={{ padding: '10px 24px', position: 'relative', zIndex: 1, ...style }}
      >
        {isActive && (
          <motion.div
  ...
  return (
    ...
    >
      ...
      {(canSeeSales || canSeePurchase || canSeeMfg || canSeeBom) && (
        <>
          <AnimatePresence>
      ...
      {(canSeeProducts || canSeeUsers || canSeeAudit) && (
        <>
          <AnimatePresence>
            {!isCollapsed && <motion.div className="sf-nav-section" style={{marginTop: '10px'}} variants={textVariants} initial="hidden" animate="visible" exit="hidden">Settings</motion.div>}
          </AnimatePresence>
          {canSeeProducts && (
            <NavItem 
              id="products" 
